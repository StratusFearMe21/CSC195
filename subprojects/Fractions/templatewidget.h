...
struct PtrFunToMemFun<P, Ret (CppClass::*)(Args...)> {
  static Ret callback(GTypeInstance *instance, Args... args) {
    auto wrapped = Glib::wrap(GTK_WIDGET(instance));
    CppClass *w = dynamic_cast<CppClass *>(wrapped);
    if (!w) {
      std::cout << "WARN: Cast failed to " << typeid(CppClass).name()
                << std::endl;
      std::cout << "WARN: type was " << typeid(wrapped).name() << std::endl;
    } else
      return (w->*P)(args...);
  }
};
...
public:
  ...
  void action_set_enabled(const Glib::ustring &name, bool enabled) {
    gtk_widget_action_set_enabled(GTK_WIDGET(this->gobj()), name.c_str(),
                                  enabled);
  }
};
